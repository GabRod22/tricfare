...
tab1, tab2, tab3 = st.tabs(["Calculator", "Algorithm", "About"])

with tab1:
    st.header('Tricycle Fare Rate :green[Calculator] :pager:')

    col1, col2 = st.columns(2)
    col1.metric("Average Gas Price Last Week", "PHP68.80")
    col2.metric("Average Gas Price Currently", "PHP66.70", "-3.05%")
...
# Initialize a flag variable to store the success message state
show_success_message = False

# Update the success message state based on the form submission
submitted = f.form_submit_button("Compute!")
        
if submitted:
    show_success_message = True

# Display the success message if the flag variable is True
if show_success_message:
    st.success(f'Your fare should be ₱{fare}', icon="✅")

    # Create a button to remove the success message
    if st.button('Remove message'):
        if isinstance(show_success_message, st.delta_generator.DeltaGenerator):
            show_success_message.empty()
...
with tab2:
    ...
    # Chart 5
        ...
        # Open the image file
        cctfr = "https://i.pinimg.com/originals/10/dd/74/10dd74daeb2eaf2e22324e97ff6293e7.jpg"

        # Display the image
        st.image(cctfr, caption='Last Updated: 2019', use_column_width=True)

    elif 'DOE Gas Price' in option:
        # Open the image file
        feb3 = "https://i.pinimg.com/originals/8e/70/91/8e70919fd62e1e1a6831760a19570ebd.png"

        # Display the image
        st.image(feb3, caption='Last Week: February 3, 2022', use_column_width=True)
      
       # Open the image file
        feb10 = "https://i.pinimg.com/originals/2f/89/68/2f8968427d3e5690e670659bc8284b7f.png"

        # Display the image
        st.image(feb10, caption='Most Recent: February 10, 2022', use_column_width=True)

with tab3:
  st.header('What is our :red[website]?')
  st.write("Our website is a practical online application designed to provide a fair and transparent fare system for tricycle transportation in Cabanatuan City. We understand that public transportation is essential for the country's development, and many people choose tricycles as their primary means of transport. We have developed a passenger and tricycle driver-based algorithm that uses Python and VS code to determine the appropriate fare rates based on the number of passengers and current gas prices.")

  st.header('Content and Features')
  st.markdown('- Our web application allows passengers to calculate the appropriate fare rate based on the number of passengers and destination landmarks')
  st.markdown('- The program provides a weekly update of gas prices to ensure that passengers and tricycle drivers are aware of the current fuel costs and can adjust their fares accordingly.')
  st.markdown('- We also offer a transparency page that visualizes the data and shows the fares for each category, such as Passenger, Landmark, Gas Price Increase, Gas Price Decrease, Classification, Fare Matrix, and, DOE Gas Price')

  st.header('Target Audience')
  st.markdown('- Our target audience is the HCPSMSHS commuters who use tricycles as their primary means of transportation.')
  st.markdown('- We aim to provide a fair and transparent fare system that benefits both passengers and tricycle drivers.')
  st.markdown('- Our program depicts the appropriate discounts for students and senior citizens/PWD to ensure that everyone can access affordable transportation.')

  
  st.header(":red[Contact Us]!")
  st.subheader("Please use the form below to send us a message")
  ...
  if st.button("Send"):
      print(f"Message sent from {name} ({email}): {message}")
